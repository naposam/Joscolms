  ...
  function fill_course($db){ 
   // $dept=$_SESSION['dept'];  
    ...
    return $output;
     ...
     function fill_level($db){ 
   // $dept=$_SESSION['dept'];  
    $output='';
    $sql="SELECT * FROM `level_tbl`";
    $result=mysqli_query($db,$sql);
    while($row = mysqli_fetch_array($result)){
    $output.='<option  value="'.$row['lid'].'">'.$row['level'].'</option>';
        }
    return $output;
    } 
...
?>
<?php 
$a=mysqli_query($db,"SELECT * FROM `option_tbl`");
$d=mysqli_num_rows($a);
$courseSelect=mysqli_query($db,"SELECT reg_opttbl.*,option_tbl.course,level_tbl.level FROM reg_opttbl left join option_tbl on reg_opttbl.course_code=option_tbl.id left join level_tbl on reg_opttbl.level=level_tbl.lid where reg_opttbl.uid='$uid'");
$count=mysqli_num_rows($courseSelect);

if(isset($_GET['id'])){
    $id=$_GET['id'];
    mysqli_query($db,"DELETE FROM reg_opttbl where rid='$id'");
    header("location: AddCourses_Optional.php?status=deleted");
}
?>
	   <!--/. NAV TOP  -->
		...
		<div id="page-wrapper" style=" min-height:500px!important;">
		  <div class="header"> 
			...
			<div class="dashboard-cards"> 
				...
				<!-- /. ROW  --> 
				<div class="row">
                    <?php require_once ('bacnkendAdd_course_Optional.php');?>
					<div class="col-xs-12 col-sm-12 col-md-12"> 
						<div class="row">
                            ...
                            $date=date('Y-m-d', strtotime($tRecords['rdate']));
                             $time=date('H:i A',strtotime($tRecords['rdate']));
                            $i++;
                                ...
                                <td><?php echo $date;?></td> 
                                <td><?php echo $time;?></td> 
                                <td><a href="AddCourses_Optional.php?id=<?php echo $tRecords['rid'];?>"><i class="fa fa-times" aria-hidden="true" style="color: red"></i></a></td> 	
                                 </tr>
