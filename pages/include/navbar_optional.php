...
$myd=mysqli_query($db,"SELECT student_tbl.*,reg_opttbl.course_code,option_tbl.course FROM student_tbl left join reg_opttbl on student_tbl.suid=reg_opttbl.uid left join option_tbl on reg_opttbl.course_code=option_tbl.id  where student_tbl.status='1' and student_tbl.level='$tlevel' and reg_opttbl.course_code='$course_ud' ");
$count=mysqli_num_rows($myd);
endif;

if(isset($_SESSION['viewID'])):
    $course_ud=$_SESSION['viewID'];
$c=mysqli_query($db,"SELECT student_tbl.*,reg_opttbl.course_code,option_tbl.course FROM student_tbl left join reg_opttbl on student_tbl.suid=reg_opttbl.uid left join option_tbl on reg_opttbl.course_code=option_tbl.id  where student_tbl.status='0' and student_tbl.level='$tlevel' and reg_opttbl.course_code='$course_ud'");
