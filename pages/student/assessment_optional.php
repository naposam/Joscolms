...
if(isset($_POST['upload']))
{
  ...
  $nameCheck=mysqli_query($db,"SELECT * from ass_student where  uid='$user_id' and ass_title='$ass_title'");
  $ass_titleChecker=mysqli_num_rows($nameCheck);

  if($ass_titleChecker<1){
...
VALUES('$ass_title','$image','$dept','$VID','$clevel','$user_id')";
  mysqli_query($db,$insertqry);
  header("location: assessment_optional.php?status=Sucessmessage");
  }
}else{
  header("location: assessment_optional.php?status=nameError");
}
}
?>
<?php endif;?>
            ...
            <div class="panel panel-primary">
                ...
                <div class="panel-body" style="height: 100%; min-height: 514px; background-color: #e2e2e2">
                    <div class=" messagealert" id="del_container" >
                      <?php
                        $up="SELECT ass.*,option_tbl.course FROM ass LEFT JOIN option_tbl ON ass.course=option_tbl.id  where ass.level='$levelID' and ass.course='$VID'";
                       ...
                       ?>
                        ...
                        <div>
                          ...
                          $sub_time=date('H:i:s', strtotime($row['sub_time'])); 
                                    ...
                                    <li class="plan">
                                        <ul class="planContainer">
                                            <li style="margin: 10px">
                                                ...
                                                <a href="temp_optional.php?viewID=<?php echo $row['ass_id']?>">
                                              <?php echo $row['ass_title']?>
                                             </a>
                                             <br><br>
                                             <a href="temp_optional.php?viewID=<?php echo $row['ass_id']?>" style="text-decoration: none;color:darkblue;font-size: 15px;">
                                                    <?php echo $row['course']?>
...
<script src="../../dist/js/jquery-2.1.4.js" type="text/javascript"></script>
 ...
 <?php if (isset($_GET['status']) && $_GET['status'] === "nameError") : ?>
   toastr.error("Assignment Already submitted Visit your dashboard to view all submited assignment. Thank you!!!");
    <?php endif;?>

    <?php if (isset($_GET['status']) && $_GET['status'] === "Sucessmessage") : ?>
   toastr.success("Assignment submitted Successfully.Visit your dashboard to View");
    <?php endif;?>
