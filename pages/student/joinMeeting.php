...
$a=mysqli_query($db,"SELECT * FROM `meetingtbl` where dept='$dept'");
$count=mysqli_num_rows($a);

$result1=mysqli_query($db,"SELECT meetingtbl.*,coursetbl.course as dcourse FROM meetingtbl left join coursetbl
 on meetingtbl.cid=coursetbl.cid where meetingtbl.dept='$dept'");
...
<?php include ('index_sidebar.php');?>
		...
		<div id="page-wrapper" style=" min-height:500px!important;">
		  <div class="header" > 
					  ...
					  <?php if($count<1):?>
				<marquee><h3 style="text-transform: uppercase;"><?php echo "There is no meeting going on currently";?></h3></marquee>
				<?php endif;?>		
					</ol> 
                    ...
                    <?php
                          $i=0; 
                            while ($row1=mysqli_fetch_array($result1)){
                               ...
                               $date=date('Y-m-d', strtotime($row1['date_posted']));
                                $time=date('H:i A',strtotime($row1['date_posted']));  
                             ?>
                         <tr class="odd gradeX">
                           <td><?php echo $i;?></td>
                            <td><?php echo $row1['dcourse'];?></td>
                            
                            <td><a href="<?php echo $row1["course_link"]?>" target="_blank">JOIN MEETING</a></td>
                            <td><?php echo $date;?></td>
                            <td><?php echo $time;?></td>
