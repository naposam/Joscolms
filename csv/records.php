...
if(isset($_GET['id'])){
  $id=$_GET['id'];
  mysqli_query($db,"UPDATE student_tbl set account=1 where suid='$id'");
  header("location: records.php?status=blocked");
}
//unblocking students
if(isset($_GET['ubid'])){
  $ubid=$_GET['ubid'];
  mysqli_query($db,"UPDATE student_tbl set account=0 where suid='$ubid' and account=1");
   header("location: records.php?status=Unblock");
}
...
<!-- <script src="dist/js/jquery-2.1.4.js" type="text/javascript"></script> -->
    ...
    <?php if (isset($_GET['status']) && $_GET['status'] === "blocked") : ?>
   toastr.warning("User Blocked Successfully");
    <?php endif;?>

    <?php if (isset($_GET['status']) && $_GET['status'] === "Unblock") : ?>
